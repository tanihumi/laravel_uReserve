          ...
          <form method="POST" action="{{ route('events.update', ['event' => $event->id]) }}">
            ...
            <div class="md:flex justify-between mt-4">
              <div>
                ...
                <x-input id="event_date" class="block mt-1 w-full" type="text" name="event_date" value="{{ $eventDate }}" required autofocus />
              </div>

              <div>
                <x-label for="start_time" value="開始時間" />
                <x-input id="start_time" class="block mt-1 w-full" type="text" name="start_time" value="{{ $startTime }}" required autofocus />
              </div>

              <div>
                <x-label for="end_time" value="終了時間" />
                <x-input id="end_time" class="block mt-1 w-full" type="text" name="end_time" value="{{ $endTime }}" required autofocus />
              </div>
            </div>
